...
@registry.register_model("gemnet_oc_trans")
class GemNetOC(BaseModel):
    ...
    def __init__(
        ...
        **kwargs,  # backwards compatibility with deprecated arguments
        ...
        if kwargs.get("gemnet_oc_freeze_until", None):
            freeze_until = kwargs["gemnet_oc_freeze_until"]
            for i in range(freeze_until, self.num_blocks + 1):
                for name, param in self.out_blocks[i].named_parameters():
                    param.requires_grad = True
            # the number of interaction blocks is num_blocks
        ...
        if self.attn_type == "base":
            # if we are not using any attention
            # we should keep these projection layers
            out_mlp_E = [
                Dense(
    ...
    def forward(self, data):
        ...
        if self.freeze:
            for i in range(self.after_freeze_numblocks):
                ...
                E, F = self.after_freeze_OB[i](h, m, basis_output, idx_t)
                F_st += self.after_out_mlp_F[i](F)
                E_t += self.after_out_mlp_E[i](E)
