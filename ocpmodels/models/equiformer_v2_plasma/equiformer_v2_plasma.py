...
@registry.register_model("equiformer_v2_plasma_newdist_direct")
class EquiformerV2_plasma(BaseModel):
    ...
    def forward(self, data):
        ...
        data = data.clone()
        data.natoms = data.natoms_gh.clone()
        data.atomic_numbers = data.atomic_numbers_gh.clone()
        data.pos = data.pos_gh.clone()
        data.tags = data.tags_gh.clone()
        data.fixed = data.fixed_gh.clone()
        data.force = data.force_gh.clone()
        data.edge_index = data.edge_index_gh.clone()
        data.cell_offsets = data.cell_offsets_gh.clone()

        # modify the data_object.batch and data_object.ptr
        batch_real = []
        ptr_real = [0]
        count = 0
        for number in data.natoms.tolist():
            batch_real = batch_real + [count] * number
            ptr_real.append(ptr_real[-1] + number)
            count += 1

        data.batch = torch.tensor(batch_real).to(
            dtype=data.batch.dtype, device=data.batch.device
        )
        data.ptr = torch.tensor(ptr_real).to(
            dtype=data.ptr.dtype, device=data.ptr.device
        )
    ...
    def generate_graph(
        ...
        if use_pbc:
            if otf_graph:
                edge_index, cell_offsets, neighbors = radius_graph_pbc_plasma(
                    data,
                    cutoff,
                    max_neighbors,
                    enforce_max_neighbors_strictly,
            ...
            raise NotImplementedError("Must Turn on use_pbc for now!!!")
            if otf_graph:
...
def radius_graph_pbc_plasma(
    ...
    # for locating proton, the atomic numbers of the atoms
    ...
    # Make sure pairs including proton (id = 0) are included
    # also, exclude the the pairs between offseted other element and proton
    mask_with_proton = (atomic_numbers[index1] == 0) | (
        atomic_numbers[index2] == 0
    )
    # Find the smallest distance for each pair, among the offset cells
    mask_smallest_dist, _ = torch.sum((pos1 - pos2) ** 2, dim=1).min(-1)
    mask_smallest_dist = mask_smallest_dist.unsqueeze(-1).repeat(1, num_cells)
    mask_smallest_dist = mask_smallest_dist == torch.sum(
        (pos1 - pos2) ** 2, dim=1
    )
    mask_smallest_dist = mask_smallest_dist.reshape(-1)

    mask_with_proton = mask_with_proton & mask_smallest_dist
    # Remove pairs that are too far apart, while keeping pairs with proton
    mask_within_radius = (
        torch.le(atom_distance_sqr, radius * radius) | mask_with_proton
    )
    ...
    # now we need to add back the pairs with proton
    mask_with_proton = (atomic_numbers[index1] == 0) | (
        atomic_numbers[index2] == 0
    )
    mask_num_neighbors = mask_num_neighbors | mask_with_proton
    ...
    unit_cell = unit_cell.view(-1, 3)
    atom_distance_sqr = torch.masked_select(
        atom_distance_sqr, mask_num_neighbors
    )

    # use the get_max_neighbors_mask function again to update num_neighbors_image
    mask_num_neighbors, num_neighbors_image = get_max_neighbors_mask(
        natoms=data.natoms,
        index=index1,
        atom_distance=atom_distance_sqr,
        max_num_neighbors_threshold=100000,  # just a large number
        enforce_max_strictly=enforce_max_neighbors_strictly,
    )
